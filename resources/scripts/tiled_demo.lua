...
function leo_update(dt)
	if not player.alive then
		...
		if
			(leo_is_key_released and leo_is_key_released(leo_KEY_R))
			or (
				leo_is_gamepad_available
				and leo_is_gamepad_available(0)
				and leo_is_gamepad_button_pressed
				and leo_is_gamepad_button_pressed(0, leo_GAMEPAD_BUTTON_FACE_DOWN)
			)
		then
	...
	if leo_is_gamepad_available and leo_is_gamepad_available(0) then
		if leo_is_gamepad_button_down then
			if leo_is_gamepad_button_down(0, leo_GAMEPAD_BUTTON_DPAD_LEFT) then
				dx = -1
			end
			if leo_is_gamepad_button_down(0, leo_GAMEPAD_BUTTON_DPAD_RIGHT) then
				dx = 1
			end
			if leo_is_gamepad_button_down(0, leo_GAMEPAD_BUTTON_DPAD_UP) then
				dy = -1
			end
			if leo_is_gamepad_button_down(0, leo_GAMEPAD_BUTTON_DPAD_DOWN) then
				dy = 1
			end
		end
	...
	if
		(leo_is_key_pressed and leo_is_key_pressed(leo_KEY_ESCAPE))
		or (
			leo_is_gamepad_available
			and leo_is_gamepad_available(0)
			and leo_is_gamepad_button_pressed
			and leo_is_gamepad_button_pressed(0, leo_GAMEPAD_BUTTON_START)
		)
	then
	...
	if
		(leo_is_key_pressed and leo_is_key_pressed(leo_KEY_SPACE))
		or (
			leo_is_gamepad_available
			and leo_is_gamepad_available(0)
			and leo_is_gamepad_button_pressed
			and leo_is_gamepad_button_pressed(0, leo_GAMEPAD_BUTTON_FACE_LEFT)
		)
	then
		if coin_sound and leo_play_sound then
	...
	for _, enemy in ipairs(enemies) do
		...
		if enemy.x < enemy.origin_x - max_distance then
			enemy.x = enemy.origin_x - max_distance
		end
		if enemy.x > enemy.origin_x + max_distance then
			enemy.x = enemy.origin_x + max_distance
		end
		if enemy.y < enemy.origin_y - max_distance then
			enemy.y = enemy.origin_y - max_distance
		end
		if enemy.y > enemy.origin_y + max_distance then
			enemy.y = enemy.origin_y + max_distance
		end
		...
		if leo_check_collision_recs then
			...
			if distance_sq <= 64 * 64 then
				...
				local rect_collision = leo_check_collision_recs(
					player_rect[1],
					player_rect[2],
					player_rect[3],
					player_rect[4],
				...
				if leo_check_collision_circles then
					...
					circle_collision = leo_check_collision_circles(
						player_center_x,
						player_center_y,
						player.size / 2,
						enemy_center_x,
				...
				if leo_check_collision_point_rec then
					local player_center_x = player.x + player.size / 2
					local player_center_y = player.y + player.size / 2
					point_collision = leo_check_collision_point_rec(
						player_center_x,
						player_center_y,
						enemy.x,
						enemy.y,
	...
	if leo_draw_text then
		...
		if leo_is_gamepad_available and leo_is_gamepad_available(0) then
			...
			if leo_get_gamepad_name then
				...
				if name then
					gamepad_text = gamepad_text .. " (" .. name .. ")"
				end
			end
		else
