...
function leo_update(dt)
    ...
    if player.x > 1260 then player.x = 1260 end
    if player.y < 20 then player.y = 20 end
    if player.y > 700 then player.y = 700 end
    ...
    for _, ball in ipairs(balls) do
        ...
        if ball.x <= ball.radius or ball.x >= 1280 - ball.radius then
            ...
            if ball.x <= ball.radius then ball.x = ball.radius end
            if ball.x >= 1280 - ball.radius then ball.x = 1280 - ball.radius end
        end
        if ball.y <= ball.radius or ball.y >= 720 - ball.radius then
            ball.vel_y = -ball.vel_y
            if ball.y <= ball.radius then ball.y = ball.radius end
            if ball.y >= 720 - ball.radius then ball.y = 720 - ball.radius end
        end
    end
...
function leo_render()
    ...
    for i = 1, 5 do
        ...
        leo_draw_rectangle(math.floor(player.x - 15 - i*2), math.floor(player.y - 15 - i*2), 
                          30 + i*4, 30 + i*4, 255, 255, 0, alpha)
    end
